...
public class CompassActivity extends Activity implements SensorEventListener {
	...
	private void setUpMapIfNeeded() {
		if (mMap == null) {
			...
			checkedItems = getIntent().getExtras().getParcelableArrayList(SecondTaskAndroid.STRING_CHECKED_ITEMS);
		...
		private void drawLines(Location location) {
			for (Item item : checkedItems) {
				mMap.addPolyline(new PolylineOptions()
						.add(new LatLng(location.getLatitude(), location.getLongitude()), new LatLng(item.getLatitude(), item.getLongitude()))
						.width(5).color(Color.parseColor(item.getColor())));
			}
		}
